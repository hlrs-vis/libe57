...
void main(int argc, char** argv)
{
	...
	try {
        ...
        cerr << "ERROR: Got an exception" << endl;
    }
}
...
void usage(ustring msg)
{
    ...
    cerr << "        e57fields scan0001.e57 /images3D/0/points" << endl;
    cerr << endl;
}
...
struct CVElementInfo {
    ...
    /// For example:  "/images3d/0/points" + "/" + recordNumber + "/" + "cartesianX"
                ...
                ImageFile imf, Statistics& stats, vector<SourceDestBuffer>& destBuffers)
{
    ...
    for (unsigned i = 0; i < allNodes.size(); i++) {
        ...
        pair<ustring,ustring> fieldAndType(fieldName, typeName);
        pair<ustring,ustring> fieldAndDistinctType(fieldName, distinctTypeName);
...
void gatherStats(Node n, Statistics& stats, ImageFile imf)
{
    ...
    /// Get OccurrenceStats for combination of fieldName and type (e.g. "cartesianX" and "Integer")
    pair<ustring,ustring> fieldAndType(fieldName, typeName);
    OccurrenceStats* fieldTypeOccur = &stats.fieldTypeStats[fieldAndType];

    /// Get OccurrenceStats for combination of fieldName and distinct type (e.g. "cartesianX" and "Integer(minimum=0,maximum=1023)")
    pair<ustring,ustring> fieldAndDistinctType(fieldName, distinctTypeName);
...
void printExtrema(const StatEntry* stats, ustring fieldName, ustring typeName, ustring statName, ustring measure)
{
    if (stats->count == 0)
        return;
...
void printStats(CommandLineOptions& options, Statistics& stats, ImageFile imf)
{
    ...
    for (iter2=stats.fieldTypeStats.begin(); iter2 != stats.fieldTypeStats.end(); ++iter2) {
        ustring fieldName            = iter2->first.first;
    ...
    for (iter3=stats.fieldDistinctTypeStats.begin(); iter3 != stats.fieldDistinctTypeStats.end(); ++iter3) {
        ustring fieldName            = iter3->first.first;
    ...
    for (iter4=stats.fieldTypeStats.begin(); iter4 != stats.fieldTypeStats.end(); ++iter4) {
        ustring fieldName            = iter4->first.first;
        ustring typeName             = iter4->first.second;
        const OccurrenceStats* occur = &iter4->second;

        printExtrema(&occur->minimum, fieldName, typeName, "min", occur->measure);
        printExtrema(&occur->maximum, fieldName, typeName, "max", occur->measure);
        printExtrema(&occur->zero, fieldName, typeName, "zero", occur->measure);
        printExtrema(&occur->quietNan, fieldName, typeName, "qNan", occur->measure);
        printExtrema(&occur->signalingNan, fieldName, typeName, "sNan", occur->measure);
        printExtrema(&occur->infinity, fieldName, typeName, "inf", occur->measure);
        printExtrema(&occur->denormalized, fieldName, typeName, "denormal", occur->measure);
        printExtrema(&occur->negativeZero, fieldName, typeName, "-0", occur->measure);
    }
    cout << endl;
}
